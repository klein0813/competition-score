...
const hiddenPrevArrow = ref(true);
const hiddenNextArrow = ref(true);

const onSingerChange = (current) => {
  const sId = singers.value[current]._id;
  changeSinger(sId).then(() => {
    if (current === 0) {
      hiddenPrevArrow.value = true;
    } else {
      hiddenPrevArrow.value = false;
    }
    if (current === toRaw(singers.value).length - 1) {
      hiddenNextArrow.value = true;
    } else {
      hiddenNextArrow.value = false;
    }
...
const update = (isIndex) => {
  getUser().then(({ data: { data, active, scores: hscores } }) => {
    ...
    if (activeIndex === toRaw(singers.value).length - 1) {
      hiddenNextArrow.value = true;
    } else {
      hiddenNextArrow.value = false;
    }
    ...
    for (const key in scores) {
      if (Object.hasOwnProperty.call(scores, key)) {
        if (scores[key].count === judgeCount) {
          ...
          scores[key].avg = (total / count).toFixed(2).padEnd(4, '0');
        }
      }
    }
    const singerScore = scores[singerIdx.value]
  ...
  socket.on(EVENT_UPDATE_SCORE, ({ data }) => {
    ...
    if (singerIdx.value === data.singerId) {
      ...
      if (currentScore.count === judgeCount) {
        ...
        scores[singerIdx.value].total = total;
        scores[singerIdx.value].avg = avg;
...
const showAvg = computed(() => {
  return currentScore.count === judges.value.length;
})
      ...
      <div class="main">
        ...
        <div class="content" v-show="isIndex">
          ...
          <div class="score">
            ...
            <div class="score-info">
              <span>已评分 {{ currentScore.count ?? 0 }} 人/总评委 {{ judges.length }} 人</span>
            ...
            <a-carousel class="judge-arousel" :dots="false" dot-position="left" :autoplay="showAvg">
              <div class="score-item" :class="currentScore.iValues[judge._id] ? 'invalid-score' : ''" v-for="judge in judges " :key="judge._id">
                <div class="judge-info">
                  <a-avatar
                    class="judge-avatar"
                    :class="onlineJids.includes(judge._id) ? 'online' : ''"
