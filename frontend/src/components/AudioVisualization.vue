...
const props = defineProps(['audio', 'auid', 'audioSource', 'analyser']);
...
let requestAnimationFrameHander = null;
let isBegining = true;
//---
/*
...
function animate() {
  ...
  requestAnimationFrameHander = requestAnimationFrame( animate );
};
...
function init() {
  ...
  document.getElementById('audioVisualization').appendChild( canvas );
  ...
  btStart = document.getElementById( 'auid' );
...
function audioSetup( url ) {
  ...
  analyserBufferLength = props.analyser.frequencyBinCount;
...
function audioEnded() {
  cancelAnimationFrame(requestAnimationFrameHander);
  onResize();
  // isBegining = true;
  // audio.currentTime = 0;
}
...
function audioPause() {
  console.log('audioPause', audio.currentTime);
  cancelAnimationFrame(requestAnimationFrameHander);
